...
class BackgroundRemoverNode:
    ...
    def remove_background(self, image, tolerance: float = 18.0, smoothing: int = 0, smooth=None):
        ...
        pixels = self._as_rgba_array(image)
    ...
    def _process_batch(self, batch, tolerance: float, smoothing: int):
        ...
        processed = []
        for sample in batch:
            sample = self._as_rgba_array(sample)
            processed.append(self._compute_background_mask(sample, tolerance=tolerance, smoothing=smoothing))

        combined = np.stack(processed)
        return combined.astype(np.uint8)

    def _as_rgba_array(self, image):
        ...
        if arr.ndim == 3 and arr.shape[-1] == 3:
            ...
            arr = np.concatenate([np.repeat(arr, 3, axis=2), np.full(arr.shape[:2] + (1,), 255, dtype=np.uint8)], axis=2)
        elif arr.ndim != 3 or arr.shape[-1] != 4:
    ...
    def _compute_background_mask(self, pixels: np.ndarray, tolerance: float = 18.0, smoothing: int = 0):
        ...
        border_y = np.concatenate([np.arange(0, rgb.shape[0], sample_size), np.arange(rgb.shape[0] - 1, -1, -sample_size)])
        border_x = np.concatenate([np.arange(0, rgb.shape[1], sample_size), np.arange(rgb.shape[1] - 1, -1, -sample_size)])
        border_pixels = []
        for y in border_y:
            for x in border_x:
        ...
        if border_pixels.size == 0:
            background_color = rgb.mean(axis=(0, 1))
        else:
        ...
        if smoothing > 0:
            background_mask = self._smooth_mask(background_mask, smoothing)
    ...
    def _smooth_mask(self, mask: np.ndarray, radius: int) -> np.ndarray:
        if radius <= 0:
            return mask
